...
namespace CommonTools
{
	...
	public class CellPainter
	{
			...
			object data)
		{
            ...
            // dMod - Para permitir dibujar el fondo de la celda con cierto color
            if (node.BackColor.HasValue)
            {
                Rectangle r = cellRect;
                r.X = column.CalculatedRect.X;
                r.Width = column.CalculatedRect.Width;
                SolidBrush brush = new SolidBrush(node.BackColor.Value);
                dc.FillRectangle(brush, r);
                brush.Dispose();
            }
            //
			...
			if (data != null)
			{
                ...
                Color color = (node.ForeColor == null ? format.ForeColor : node.ForeColor.Value);
                Font font = (node.Font == null ? m_owner.Font : node.Font);
                // d
                ...
                TextRenderer.DrawText(dc, data.ToString(), font, cellRect, color, flags);
                // d
			}
		}
