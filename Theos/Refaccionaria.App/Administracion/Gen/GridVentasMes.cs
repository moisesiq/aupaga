...
namespace Refaccionaria.App
{
    public partial class GridVentasMes : UserControl
    {
        ...
        public void LlenarDatos(int iParteID)
        {
            ...
            var oDatos = Datos.ExecuteProcedure<pauParteVentasPorMes_Result>("pauParteVentasPorMes", oParams);           
            ...
            foreach (var oReg in oDatos)
            {
                ...
                if (oReg.Anio < iAnio)
                {
                    ...
                    this.dgvDatos[syear, iFila].Value = (Util.Decimal(this.dgvDatos[syear, iFila].Value)
                        + oReg.Cantidad.Valor());
                }
            ...
            for (int iCol = 1; iCol < this.dgvDatos.Columns.Count; iCol++)
            {
                ...
                int iColumna = Util.Entero(this.dgvDatos.Columns[iCol].Name);
                
                foreach (DataGridViewRow oFila in this.dgvDatos.Rows)
                {                                              
					string sValor = Util.Cadena(oFila.Cells[iCol].Value);                        
					var aValores = sValor.Split('/');
					if (aValores.Length > 0)
					{
						mCantidad += Util.Decimal(aValores[0]);
					}
					if (aValores.Length > 1)
					{
						mNegado += Util.Decimal(aValores[1]);
					}
                }
