    ...
    public partial class CuentasContables : UserControl
    {
        ...
        private void dgvGastos_CurrentCellChanged(object sender, EventArgs e)
        {
            if (this.dgvGastos.VerSeleccionNueva())
            {
                ...
                {
                    int iEgresoID = Util.Entero(this.dgvGastos.CurrentRow.Cells["Gastos_ContaEgresoID"].Value);
        ...
        {
            decimal mTotal = Util.Decimal(oNodo.Cells[ColsCuentas.Total].Value);
            decimal mImporteDev = Util.Decimal(oNodo.Cells[ColsCuentas.ImporteDev].Value);
        ...
        private void LlenarMovimientosCuenta(int iCuentaID)
        {
            ...
            var oMovs = Datos.GetListOf<ContaEgresosView>(c => c.ContaCuentaAuxiliarID == iCuentaID && (c.Fecha >= dDesde && c.Fecha < dHasta))
                .OrderBy(c => c.Fecha);
        ...
        {
            var oDevs = Datos.GetListOf<ContaEgresosDevengadosView>(c => c.ContaEgresoID == iEgresoID);
            this.dgvGastoDev.Rows.Clear();
