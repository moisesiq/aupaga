...
namespace Refaccionaria.App
{
    public partial class CuentasPolizas : UserControl
    {
            ...
            if (e.Button == MouseButtons.Right)
            {
                ...
                if (oFila != null)
                {
                    if (this.tgvCuentas.GetNodeForRow(e.RowIndex).Level == 3 &&
                        Util.Entero(oFila.Cells["Cuentas_Id"].Value) == Cat.ContaCuentasDeMayor.Clientes)
        ...
        private void FormatoColumnasImporte(TreeGridNode oNodo)
        {
            for (int iCol = 2; iCol < this.tgvCuentas.Columns.Count; iCol++)
            {
                ...
                if (oNodo.Cells[iCol].Value != null)
                    oNodo.Cells[iCol].Value = Util.Decimal(oNodo.Cells[iCol].Value).ToString(GlobalClass.FormatoMoneda);
            }
        }
        ...
        private void MovimientosCuentaFormatoAdicional()
        {
            ...
            {
                if (Util.Logico(oFila.Cells["Error"].Value))
                    oFila.DefaultCellStyle.ForeColor = Color.Red;

                if (Util.Logico(oFila.Cells["FueManual"].Value))
                {
        ...
        {
            var oDetalle = Datos.GetListOf<ContaPolizasDetalleAvanzadoView>(c => c.ContaPolizaID == iPolizaID);
            this.dgvPoliza.Rows.Clear();
        ...
        private void ValidarSaldoClientes()
        {
            ...
            {
                if (Util.Entero(oFila.Cells["Cuentas_Tipo"].Value) == NivelesArbol.CuentaDeMayor
                && Util.Entero(oFila.Cells["Cuentas_Id"].Value) == Cat.ContaCuentasDeMayor.Clientes)
            ...
            var oCuentasCli = Datos.GetListOf<ContaCuentaAuxiliar>(c => c.ContaCuentaDeMayorID == Cat.ContaCuentasDeMayor.Clientes);
            var oAdeudos = Datos.GetListOf<ClientesCreditoView>(c => c.Adeudo > 0);
            foreach (var oNodo in oNodoCli.Nodes)
            {
                int iCuentaID = Util.Entero(oNodo.Cells[ColumnasArbol.Id].Value);
                ...
                decimal mSaldo = Util.Decimal(oNodo.Cells[ColumnasArbol.Total].Value);
