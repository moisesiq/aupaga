...
namespace Refaccionaria.App
{
    public partial class ContaBancos : UserControl
    {
        ...
        private void LlenarAsignaciones()
        {
            ...
            var oPendientes = Datos.GetListOf<BancosCuentasMovimientosView>(c => (c.RelacionTabla == null || c.RelacionTabla != Cat.Tablas.VentaPagoDetalle || c.Resguardado)
        ...
        {
            int iBancoCuentaID = Util.Entero(this.cmbBancoCuenta.SelectedValue);
            if (iBancoCuentaID <= 0)
                return;
            ...
            var oMovs = Datos.GetListOf<BancosCuentasMovimientosView>(c => c.BancoCuentaID == iBancoCuentaID && c.FechaAsignado >= dDesde && c.FechaAsignado < dHasta
        ...
        private void CalcularImporteSeleccion()
        {
            ...
            foreach (DataGridViewRow oFila in this.dgvConciliacion.Rows)
            {
                ...
                {
                    mDepositos += Util.Decimal(oFila.Cells["con_Depositos"].Value);
                    mRetiros += Util.Decimal(oFila.Cells["con_Retiros"].Value);
                }
            }
        ...
        {
            var oMovs = Datos.GetListOf<BancosCuentasMovimientosView>(c => c.MovimientoAgrupadorID == iMovID);
        ...
        {
            var oMovsGrupo = Datos.GetListOf<BancoCuentaMovimiento>(c => c.MovimientoAgrupadorID == iMovAgrupadorID);
            ...
            var oMov = Datos.GetEntity<BancoCuentaMovimiento>(c => c.BancoCuentaMovimientoID == iMovAgrupadorID);
        ...
        private void GuardarConciliacion()
        {
            ...
            foreach (DataGridViewRow oFila in this.dgvConciliacion.Rows)
            {
                ...
                if (iCambio > 0)  // Hubo una modificación
                {
                    ...
                    if (iCambio == 2)
                    {
                        ...
                        oMov.FechaAsignado = Util.FechaHora(oFila.Cells["con_FechaAsignado"].Value);
                    }
            ...
            {
                if (!oFila.Cells["cnd_Sel"].ReadOnly && Util.Logico(oFila.Cells["cnd_Sel"].Value))
                {
                    int iMovID = Util.Entero(oFila.Cells["cnd_BancoCuentaMovimientoID"].Value);
