...
namespace Refaccionaria.App
{
    public partial class CuentasPorSemana : UserControl
    {
        ...
        private void cmbAnio_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            {
                this.LlenarColumnasAnio(Util.Entero(this.cmbAnio.Text));
                this.CargarDatos();
            }
        }
        ...
        private void LlenarDetalleSemana(DataGridViewCell oCelda)
        {
            ...
            bool bEspecial = Util.Logico(this.tgvDatos.GetNodeForRow(oCelda.OwningRow).Parent.Parent.Parent.Parent.Tag);

            foreach (int iId in oLista)
            {
                // Se verifica si es especial o normal
                if (bEspecial) { 
                    var oDevEsp = Datos.GetEntity<ContaEgresoDevengadoEspecial>(c => c.ContaEgresoDevengadoEspecialID == iId);
                    var oEgresoV = Datos.GetEntity<ContaEgresosView>(c => c.ContaEgresoID == oDevEsp.ContaEgresoID);
                    if (oEgresoV != null)
                    {
                ...
                {
                    var oEgresoDev = Datos.GetEntity<ContaEgresoDevengado>(c => c.ContaEgresoDevengadoID == iId);
                    var oEgresoV = Datos.GetEntity<ContaEgresosView>(c => c.ContaEgresoID == oEgresoDev.ContaEgresoID);
                    if (oEgresoV != null)
                    {
                        ...
                        this.dgvDetalle.Rows.Add(oEgresoDev.Fecha, oEgresoV.Sucursal, oEgresoV.FormaDePago, oEgresoV.Usuario, oEgresoV.Importe
                            , (Util.DividirONull(oEgresoDev.Importe, oEgresoV.Importe) * 100), oEgresoDev.Importe, oEgresoV.Observaciones);
                    }
                }
            }
        }
