...
namespace Refaccionaria.App
{
    public partial class CuentaContable : Form
    {
        ...
        private void CuentaContable_Load(object sender, EventArgs e)
        {
            ...
            this.cmbDevengarEspecial.CargarDatos("DuenioID", "Duenio1", Datos.GetListOf<Duenio>());
        ...
        private void LlenarDatos(int iCuentaID)
        {
            ...
            {
                case Tipo.Cuenta:
                    var oCuenta = Datos.GetEntity<ContaCuenta>(c => c.ContaCuentaID == iCuentaID);
                    ...
                    break;
                case Tipo.Subcuenta:
                    var oSubcuenta = Datos.GetEntity<ContaSubcuenta>(c => c.ContaSubcuentaID == iCuentaID);
        ...
        private object LlenarDatosCuentaAuxiliar(int iCuentaID)
        {
            ...
            if (oCuentaAux.DevengarAut.Valor())
            {
                ...
                var oDevAut = Datos.GetListOf<ContaCuentaAuxiliarDevengadoAutomatico>(c => c.ContaCuentaAuxiliarID == iCuentaID);
        ...
        private bool AccionGuardar()
        {
            ...
            switch (this.TipoCuenta)
            {
                case Tipo.Cuenta:
                    ...
                    Datos.Guardar<ContaCuenta>(oCuenta);
                    break;
                case Tipo.Subcuenta:
                    ...
                    Datos.Guardar<ContaSubcuenta>(oSubcuenta);
                    break;
                case Tipo.CuentaDeMayor:
                    ...
                    Datos.Guardar<ContaCuentaDeMayor>(oCuentaMay);
                    break;
                case Tipo.CuentaAuxiliar:
        ...
        private bool Validar()
        {
            ...
            if (this.chkDevengarAutomaticamente.Checked)
            {
                decimal mPorTotal = 0;
                foreach (DataGridViewRow oFila in this.dgvDevSuc.Rows)
                    mPorTotal += Util.Decimal(oFila.Cells["Porcentaje"].Value);
                if (mPorTotal == 0)
            ...
            if (this.chkDevengarEspecial.Checked)
            {
                if (Util.Entero(this.cmbDevengarEspecial.SelectedValue) == 0)
                    this.ctlError.PonerError(this.cmbDevengarEspecial, "Debes seleccionar un devengado especial.");
            }
            if (this.chkCalculoSemanal.Checked)
            {
                if (Util.Decimal(this.txtMeses.Text) <= 0)
