...
namespace Refaccionaria.App
{
    public partial class SeleccionarNotasDeCredito : UserControl
    {
        ...
        private void SeleccionarNotasDeCredito_Load(object sender, EventArgs e)
        {
            ...
            var oVenta = Datos.GetEntity<Venta>(c => c.VentaID == this.VentaID && c.Estatus);
            int iSucursalID = (oVenta == null ? 0 : oVenta.SucursalID);
            // Se cargan las notas de crédito del cliente
            var oNotas = Datos.GetListOf<NotaDeCredito>(q => q.ClienteID == this.ClienteID && (iSucursalID == 0 || q.SucursalID == iSucursalID) && q.Valida && q.Estatus);
        ...
        private void CalcularTotal()
        {
            ...
            {
                if (Util.Logico(Fila.Cells["Usar"].Value))
                    mTotal += Util.Decimal(Fila.Cells["ImporteAUsar"].Value);
            }
        ...
        public bool HayNotasDeOtrosClientes
        {
            get
            {
                foreach (DataGridViewRow Fila in this.dgvNotas.Rows)
                {
                    if (Util.Logico(Fila.Cells["Usar"].Value))
                    {
                        if (Util.Entero(Fila.Cells["colClienteID"].Value) != this.ClienteID)
                            return true;
                    }
                }
                return false;
        ...
        public Dictionary<int, decimal> GenerarNotasDeCredito()
        {
            ...
            foreach (DataGridViewRow Fila in this.dgvNotas.Rows)
            {
                if (Util.Logico(Fila.Cells["Usar"].Value) && Util.Decimal(Fila.Cells["ImporteAUsar"].Value) > 0)
                    oNotas.Add(Util.Entero(Fila.Cells["NotaDeCreditoID"].Value), Util.Decimal(Fila.Cells["ImporteAUsar"].Value));
            }
            return oNotas;
        }
        ...
        public List<int> NotasDeCreditoOtrosClientes()
        {
            ...
            foreach (DataGridViewRow Fila in this.dgvNotas.Rows)
            {
                if (Util.Logico(Fila.Cells["Usar"].Value) && Util.Entero(Fila.Cells["colClienteID"].Value) != this.ClienteID)
                    oNotas.Add(Util.Entero(Fila.Cells["NotaDeCreditoID"].Value));
            }
            return oNotas;
        }
