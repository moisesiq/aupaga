...
namespace Refaccionaria.App
{
    public partial class BusquedaGarantia : BusquedaVenta
    {
        ...
        public int SeleccionGarantiaID
        {
            get
            {
                if (this.dgvPendientes.CurrentRow == null)
                    ...
                    return Util.Entero(this.dgvPendientes.CurrentRow.Cells["penVentaGarantiaID"].Value);
            }
        }
        ...
        private void BusquedaGarantia_Load(object sender, EventArgs e)
        {
            ...
            var oMotivos = Datos.GetListOf<VentaGarantiaMotivo>();
            int iCont = 0;
        ...
        private void dgvPendientes_CurrentCellChanged(object sender, EventArgs e)
        {
            ...
            {
                int iGarantiaID = Util.Entero(this.dgvPendientes.CurrentRow.Cells["penVentaGarantiaID"].Value);
                ...
                foreach (Control oControl in this.gpbAccionPosterior.Controls)
                {
                    if (oControl is RadioButton)
                    {
                        ...
                        if (Util.Entero(oControl.Tag) == oGarantia.AccionID)
        ...
        protected override void AplicarFiltro()
        {
            ...
            {
                Lista = Datos.GetListOf<VentasView>(q => q.Folio == sFolio && (iSucursalID == 0 || q.SucursalID == iSucursalID));
            }
        ...
        private int ObtenerIdDeRadioSeleccionado(Control oContenedor)
        {
            foreach (Control oControl in oContenedor.Controls)
            {
                if (oControl is RadioButton)
                {
                    ...
                    {
                        return Util.Entero(oControl.Tag);
                    }
                }
            }
            return 0;
        ...
        public void ActualizarGarantiasPendientes()
        {
            ...
            if (this.chkPenMostrarTodas.Checked)
            {
                ...
                oDatos = Datos.GetListOf<VentasGarantiasView>(c => c.Fecha >= this.dtpPenDesde.Value.Date && c.Fecha < dHasta);
            }
            else
            {
                oDatos = Datos.GetListOf<VentasGarantiasView>(c => c.AccionID == Cat.VentasGarantiasAcciones.RevisionDeProveedor && c.EstatusGenericoID != Cat.EstatusGenericos.Completada);
            }
        ...
        public bool ValidarPendiente()
        {
            ...
            if (Util.Entero(this.dgvPendientes.CurrentRow.Cells["penEstatusGenericoID"].Value) != Cat.EstatusGenericos.Resuelto)
            ...
            if (this.rdbApCheque.Checked || this.rdbApTarjeta.Checked)
            {
                ...
                if (oGarantia != null)
                {
                    ...
                    var oVentaDet = Datos.GetListOf<VentaDetalle>(c => c.VentaID == oGarantia.VentaID && c.Estatus);
