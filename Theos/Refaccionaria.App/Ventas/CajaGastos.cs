...
namespace Refaccionaria.App
{
    public partial class CajaGastos : UserControl
    {
        ...
        private void btnAceptar_Click(object sender, EventArgs e)
        {
            ...
            int iCategoriaID = Util.Entero(this.cmbGastos.SelectedValue == null ? this.cmbOtrosIngresos.SelectedValue : this.cmbGastos.SelectedValue);
            this.dgvConceptos.Rows.Add(true, 0, iCategoriaID, sTipo, this.txtConcepto.Text, Util.Decimal(this.txtImporte.Text));
        ...
        private bool Validar()
        {
            ...
            if (!Util.ValidarDecimal(this.txtImporte.Text))
        ...
        public List<CajaIngreso> GenerarListaDeIngresos()
        {
            ...
            {
                if (Util.Logico(Fila.Cells["EsNuevo"].Value) && Util.Cadena(Fila.Cells["Tipo"].Value).ToLower() == "ingreso")
                    oLista.Add(new CajaIngreso()
                    {
                        CajaTipoIngresoID = Util.Entero(Fila.Cells["CategoriaID"].Value),
                        Concepto = Util.Cadena(Fila.Cells["Concepto"].Value),
        ...
        public List<Dictionary<string, object>> GenerarListaDeEgresos()
        {
            ...
            foreach (DataGridViewRow Fila in this.dgvConceptos.Rows)
            {
                if (Util.ConvertirBool(Fila.Cells["EsNuevo"].Value) && Util.ConvertirCadena(Fila.Cells["Tipo"].Value).ToLower() == "gasto")
                    oLista.Add(new CajaEgreso()
                    {
                        CajaTipoEgresoID = Util.ConvertirEntero(Fila.Cells["CategoriaID"].Value),
                        Concepto = Util.ConvertirCadena(Fila.Cells["Concepto"].Value),
                        Importe = Util.ConvertirDecimal(Fila.Cells["Importe"].Value),
                        Fecha = dAhora,
                        SucursalID = GlobalClass.SucursalID
                    });
            }
            */
            ...
            foreach (DataGridViewRow Fila in this.dgvConceptos.Rows)
            {
                if (Util.Logico(Fila.Cells["EsNuevo"].Value) && Util.Cadena(Fila.Cells["Tipo"].Value).ToLower() == "gasto")
                {
        ...
        public List<CajaIngreso> GenerarListaDeIngresosBorrados()
        {
            ...
            foreach (int iIngresoID in this.IngresosBorrados)
                oLista.Add(Datos.GetEntity<CajaIngreso>(q => q.CajaIngresoID == iIngresoID && q.Estatus));
            return oLista;
        }
        ...
        public List<CajaEgreso> GenerarListaDeEgresosBorrados()
        {
            ...
            foreach (int iEgresoID in this.EgresosBorrados)
                oLista.Add(Datos.GetEntity<CajaEgreso>(q => q.CajaEgresoID == iEgresoID && q.Estatus));
            return oLista;
        }
        ...
        public void ActualizarDatos()
        {
            ...
            var oCuentasAux = Datos.GetListOf<ContaCuentasAuxiliaresView>().Where(c => c.VisibleEnCaja).Select(c =>
            ...
            this.cmbOtrosIngresos.CargarDatos("CajaTipoIngresoID", "NombreTipoIngreso"
                , Datos.GetListOf<CajaTipoIngreso>(q => q.Seleccionable && q.Estatus).OrderBy(c => c.NombreTipoIngreso).ToList());

            // Se llenan los movimientos del día
            DateTime dHoy = DateTime.Today;
            var oEgresos = Datos.GetListOf<CajaEgreso>(q => q.SucursalID == GlobalClass.SucursalID && EntityFunctions.TruncateTime(q.Fecha) == dHoy && q.Estatus);
            var oIngresos = Datos.GetListOf<CajaIngreso>(q => q.SucursalID == GlobalClass.SucursalID && EntityFunctions.TruncateTime(q.Fecha) == dHoy && q.Estatus);
