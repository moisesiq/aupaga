...
namespace Refaccionaria.App
{
    public partial class Autorizaciones : UserControl
    {
        ...
        {
            var oTiposAut = Datos.GetListOf<AutorizacionProceso>(q => q.Estatus);
        ...
        {
            int iAutProcesoID = Util.Entero(this.cmbTipo.SelectedValue);
            var oAutorizaciones = Datos.GetListOf<AutorizacionesView>(q =>
                !q.Autorizado
        ...
        private void LlenarDetalle(int iAutorizacionID)
        {
            ...
            switch (oAut.AutorizacionProcesoID)
            {
                ...
                case Cat.AutorizacionesProcesos.c9500SinAnticipo:
                    var o9500V = Datos.GetEntity<Cotizaciones9500View>(q => q.Cotizacion9500ID == oAut.TablaRegistroID);
                    if (o9500V == null) { bNoEncontrado = true; break; }
                    var o9500DetalleV = Datos.GetListOf<Cotizaciones9500DetalleView>(q => q.Cotizacion9500ID == o9500V.Cotizacion9500ID);
                ...
                case Cat.AutorizacionesProcesos.CorteDeCaja:
                    var oCorte = Datos.GetEntity<CajaEfectivoPorDiaView>(q => q.CajaEfectivoPorDiaID == oAut.TablaRegistroID);
                ...
                case Cat.AutorizacionesProcesos.CreditoNoAplicable:
                    var oVentaCNA = Datos.GetEntity<VentasView>(q => q.VentaID == oAut.TablaRegistroID);
                    if (oVentaCNA == null) { bNoEncontrado = true; break; }
                    var oClienteCredito = Datos.GetEntity<ClientesCreditoView>(q => q.ClienteID == oVentaCNA.ClienteID);
                ...
                case Cat.AutorizacionesProcesos.DevolucionCancelacion:
                    var oDev = Datos.GetEntity<VentasDevolucionesView>(q => q.VentaDevolucionID == oAut.TablaRegistroID);
                    if (oDev == null) { bNoEncontrado = true; break; }
                    var oVentaDC = Datos.GetEntity<VentasView>(q => q.VentaID == oDev.VentaID);
                    var oDevDet = Datos.GetListOf<VentasDevolucionesDetalleView>(q => q.VentaDevolucionID == oDev.VentaDevolucionID);
                ...
                case Cat.AutorizacionesProcesos.Garantia:
                    var oGarantiaV = Datos.GetEntity<VentasGarantiasView>(c => c.VentaGarantiaID == oAut.TablaRegistroID);
                    if (oGarantiaV == null) { bNoEncontrado = true; break; }
                    var oVentaGC = Datos.GetEntity<VentasView>(q => q.VentaID == oGarantiaV.VentaID);
                ...
                case Cat.AutorizacionesProcesos.FondoDeCajaDiferencia:
                    var oFondo = Datos.GetEntity<CajaEfectivoPorDiaView>(q => q.CajaEfectivoPorDiaID == oAut.TablaRegistroID);
                    if (oFondo == null) { bNoEncontrado = true; break; }
                    // Se obtiene el registro del día anterior, para obtener el importe del cierre
                    var oDias = Datos.GetListOf<CajaEfectivoPorDia>(q => q.CajaEfectivoPorDiaID < oFondo.CajaEfectivoPorDiaID 
                        && q.SucursalID == oAut.SucursalID && q.Estatus);
                    int iAnteriorID = (oDias.Count > 0 ? oDias.Max(r => r.CajaEfectivoPorDiaID) : 0);
                    var oCorteAnt = Datos.GetEntity<CajaEfectivoPorDiaView>(q => q.CajaEfectivoPorDiaID == iAnteriorID);
                    //
                ...
                case Cat.AutorizacionesProcesos.Resguardo:
                    var oGastoV = Datos.GetEntity<CajaEgresosView>(q => q.CajaEgresoID == oAut.TablaRegistroID);
                    ...
                    var oGasto = Datos.GetEntity<CajaEgreso>(c => c.CajaEgresoID == oGastoV.CajaEgresoID);
                    if (oGasto.ContaEgresoID > 0)
                    {
                        var oContaEgreso = Datos.GetEntity<ContaEgreso>(c => c.ContaEgresoID == oGasto.ContaEgresoID);
                        var oCuentaAuxV = Datos.GetEntity<ContaCuentasAuxiliaresView>(c => c.ContaCuentaAuxiliarID == oContaEgreso.ContaCuentaAuxiliarID);
                ...
                case Cat.AutorizacionesProcesos.Refuerzo:
                    var oIngreso = Datos.GetEntity<CajaIngresosView>(q => q.CajaIngresoID == oAut.TablaRegistroID);
                ...
                case Cat.AutorizacionesProcesos.NotaDeCreditoCrear:
                    var oNotaV = Datos.GetEntity<NotasDeCreditoView>(q => q.NotaDeCreditoID == oAut.TablaRegistroID);
                ...
                case Cat.AutorizacionesProcesos.VentaCambio:
                    var oVentaCambio = Datos.GetEntity<VentasCambiosView>(q => q.VentaCambioID == oAut.TablaRegistroID);
                ...
                case Cat.AutorizacionesProcesos.NotaDeCreditoFiscalCrear:
                    var oNotaFiscalV = Datos.GetEntity<NotasDeCreditoFiscalesView>(c => c.NotaDeCreditoFiscalID == oAut.TablaRegistroID);
