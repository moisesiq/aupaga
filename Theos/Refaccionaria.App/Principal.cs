    ...
    public partial class Principal : Form
    {
        ...
        protected override bool ProcessCmdKey(ref Message msg, Keys keyData)
        {
            switch (keyData)
            {
                ...
                default:
                    var oControl = Util.ControlAlFrente(this.panelContenedor);
        ...
        private void Principal_Load(object sender, EventArgs e)
        {
            ...
            //    {
            //        usuarioC.Contrasenia = Util.Encriptar(usuarioC.Contrasenia.ToLower());
        ...
        private void btnContabilidad_Click(object sender, EventArgs e)
        {
            ...
            if (!UtilLocal.ValidarPermiso("Contabilidad.Acceso", true))
                return;
            //
        ...
        private void improveThread()
        {
            try
            {
                var init = Datos.GetEntity<ParteEstatus>(p => p.Estatus);
            }
            catch (Exception ex)
            {
                Util.MensajeError(string.Format("msj:{0}. inner:{1}.", ex.Message, ex.InnerException), GlobalClass.NombreApp);
            }
        }
                ...
                if (oItem is ToolStripStatusLabel)
                {
                    if (oItem.Name.StartsWith("sslAcc_"))
                    {
                        ...
                        oItem.Click += new EventHandler((s, e) =>
                        {
                            if (oBoton.Tag != null)
                            {
                                string sComando = Util.Cadena(oBoton.Tag);
        ...
        public void MostrarMensaje(object o)
        {
            ...
            else
            {
                ...
                Util.MensajeInformacion(oMensaje.Valor2, oMensaje.Valor1);
            }
        }
