/* Script con modificaciones a la base de datos de Theos. Archivo 003
 * Creado: 2015/04/17
 * Subido: 2015/04/23
 */
...
UPDATE ContaConfigAfectacion SET Operacion = 'Devolución de Venta Ticket con Vale' WHERE ContaConfigAfectacionID = 27
INSERT INTO ContaConfigAfectacion (Operacion, ContaTipoPolizaID) VALUES
	('Garantía de Venta Ticket con Vale', 1)
	, ('Pago a Proveedor con Descuento Factura', 1)
...
ALTER VIEW [dbo].[ProveedoresNotasDeCreditoView] AS
	SELECT
		...
		, ISNULL(SUM(pd.Importe), 0.0) AS Usado
		, ((ndc.Subtotal + ndc.Iva) - ISNULL(SUM(pd.Importe), 0.0)) AS Restante
		, ndc.OrigenID
		, ndco.Origen
		, ndc.Observacion
		, ndc.Disponible
