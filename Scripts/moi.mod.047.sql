/* Script con modificaciones a la base de datos de Theos. Archivo 047
 * Creado: 2015/11/11
 * Subido: 2015/11/11
 */
...
ALTER TABLE ParteEquivalente DROP CONSTRAINT DF_ParteEquivalente_Estatus, DF_ParteEquivalente_Actualizar
DROP INDEX Ix_Estatus ON ParteEquivalente
ALTER TABLE ParteEquivalente DROP COLUMN Estatus, Actualizar
...
ALTER VIEW [dbo].[PartesEquivalentesView] AS
	...
	FROM
		ParteEquivalente pe
		INNER JOIN ParteEquivalente per ON per.GrupoID = pe.GrupoID AND per.ParteID != pe.ParteID
				...
				SELECT DISTINCT ParteID FROM (
					...
					FROM
						(
							...
							FROM @Partes p INNER JOIN ParteEquivalente pe ON pe.ParteID = p.ParteID
						) p
