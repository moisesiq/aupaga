/* Script con modificaciones para el módulo de ventas. Archivo 93
 * Creado: 2015/02/25
 * Subido: 2015/03/05
 */
...
BEGIN TRY
	...
	ALTER TABLE ContaCuentaAuxiliar ALTER COLUMN CuentaContpaq NVARCHAR(16) NULL

	ALTER TABLE CajaEfectivoPorDia ADD
		FacturaGlobalRestar DECIMAL(12, 2) NULL
		, FacturaGlobalRestado BIT NULL

	INSERT INTO Configuracion (Nombre, Valor, ValorPredeterminado, Descripcion, SucursalID) VALUES
		('Ventas.FacturaGlobal.Restar', '0', '0', '', 1)
		, ('Ventas.FacturaGlobal.Restar', '0', '0', '', 2)
		, ('Ventas.FacturaGlobal.Restar', '0', '0', '', 3)
...
ALTER TABLE ContaPolizaDetalle ADD
	Cargo DECIMAL(12, 2) NULL
	, Abono DECIMAL(12, 2) NULL
GO
UPDATE ContaPolizaDetalle SET
	Cargo = CASE WHEN EsCargo = 1 THEN Importe ELSE 0.0 END
	, Abono = CASE WHEN EsCargo = 0 THEN Importe ELSE 0.0 END
ALTER TABLE ContaPolizaDetalle DROP COLUMN Importe, EsCargo
ALTER TABLE ContaPolizaDetalle ALTER COLUMN	Cargo DECIMAL(12, 2) NOT NULL
ALTER TABLE ContaPolizaDetalle ALTER COLUMN Abono DECIMAL(12, 2) NOT NULL
...
ALTER VIEW [dbo].[VentasPagosDetalleView] AS
	SELECT
...
-- DROP VIEW VentasPagosDetalleAvanzadoView
CREATE VIEW VentasPagosDetalleAvanzadoView AS
	SELECT
		vpd.VentaPagoDetalleID
		, vpd.TipoFormaPagoID AS FormaDePagoID
		, tfp.NombreTipoFormaPago AS FormaDePago
...
DROP VIEW [dbo].[VentasFacturasDetalleView]
GO

CREATE VIEW VentasFacturasDetalleAvanzadoView AS
	SELECT
...
ALTER VIEW [dbo].[ContaPolizasDetalleView] AS
	SELECT
