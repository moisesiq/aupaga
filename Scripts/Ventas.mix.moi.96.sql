/* Script con modificaciones para el módulo de ventas. Archivo 96
 * Creado: 2015/03/27
 * Subido: 2015/03/30
 */
...
BEGIN TRY
	...
	UPDATE ContaConfigAfectacion SET Operacion = 'Gasto Caja Facturado' WHERE ContaConfigAfectacionID = 19
	INSERT INTO ContaConfigAfectacion (Operacion, ContaTipoPolizaID) VALUES
		('Gasto Contable Facturado Efectivo', 1)
		, ('Gasto Contable Facturado Banco', 1)
		, ('Gasto Contable Nota Efectivo', 1)

	ALTER TABLE ContaEgreso ADD FolioFactura NVARCHAR(16) NULL
...
DROP VIEW ContaPolizasDetalleView
GO

CREATE VIEW ContaPolizasDetalleAvanzadoView AS
	SELECT
...
-- DROP PROCEDURE pauContaCuentasPolizas
CREATE PROCEDURE pauContaCuentasPolizas (
	@Desde DATE
	, @Hasta DATE
) AS BEGIN
	SET NOCOUNT ON
	...
	FROM
		...
		LEFT JOIN (
			SELECT
				...
				, cp.Fecha
				, cp.SucursalID
				, (cpd.Abono + cpd.Abono) AS Importe
			FROM
				ContaPolizaDetalle cpd
