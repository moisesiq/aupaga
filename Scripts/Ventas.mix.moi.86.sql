/* Script con modificaciones para el módulo de ventas. Archivo 86
 * Creado: 2015/02/02
 * Subido: 2015/02/06
 */
...
BEGIN TRY
	...
	UPDATE ParteExistencia SET Existencia = 0 
		FROM ParteExistencia pe INNER JOIN Parte p ON p.ParteID = pe.ParteID AND p.Estatus = 1
		WHERE pe.Estatus = 1 AND pe.Existencia != 0 AND p.EsServicio = 1
	-- Correr migración kárdex
	-- Correr cuadrar existencias
...
SELECT 
	...
	, (MovimientoInventario.ImporteTotal - ISNULL(ppdc.Importe, 0.0)) AS Saldo
...
SET NOCOUNT ON
	...
	;WITH _Pedidos AS (
		SELECT
			...
			,ExistenciaSuc02
			,ExistenciaSuc03
			, CASE WHEN (MinMatriz >= ExistenciaMatriz AND (MaxMatriz - ExistenciaMatriz - TraspasoMatriz) > 0)
				THEN MaxMatriz - ExistenciaMatriz - TraspasoMatriz ELSE 0.0 END AS NecesidadMatriz
			, CASE WHEN (MinSuc02 >= ExistenciaSuc02 AND (MaxSuc02 - ExistenciaSuc02 - TraspasoSuc02) > 0)
				THEN MaxSuc02 - ExistenciaSuc02 - TraspasoSuc02 ELSE 0.0 END AS NecesidadSuc02
			, CASE WHEN (MinSuc03 >= ExistenciaSuc03 AND (MaxSuc03 - ExistenciaSuc03 - TraspasoSuc03) > 0)
				THEN MaxSuc03 - ExistenciaSuc03 - TraspasoSuc03 ELSE 0.0 END AS NecesidadSuc03
