...
namespace Refaccionaria.App
{
    public partial class CompraDePedido : Form
    {
        ...
        {
            var oPedido = General.GetListOf<PedidosDetalleView>(c => c.PedidoID == iPedidoID && c.PedidoEstatusID == Cat.PedidosEstatus.NoSurtido);
        ...
        private bool AccionAceptar()
        {
            ...
            foreach (DataGridViewRow oFila in this.dgvDatos.Rows)
            {
                if (Helper.ConvertirBool(oFila.Cells["Sel"].Value))
                {
                    ...
                    oReg.CantidadPedido = Helper.ConvertirDecimal(oFila.Cells["Cantidad"].Value);
                    oReg.CostosUnitario = Helper.ConvertirDecimal(oFila.Cells["Costo"].Value);
