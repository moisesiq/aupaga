...
namespace Refaccionaria.App
{
    public partial class CuadroCobranza : UserControl
    {
        ...
        private void CargarDatos()
        {
            ...
            var oClientes = General.ExecuteProcedure<pauCuadroDeControlClientesCredito_Result>("pauCuadroDeControlClientesCredito", oParams);
            var oDatos = General.ExecuteProcedure<pauCuadroDeControlCobranza_Result>("pauCuadroDeControlCobranza", oParams);
            ...
            foreach (var oReg in oClientes)
            {
                ...
                this.dgvClientes.Rows.Add(oReg.ClienteID, oReg.Cliente, oReg.DiasDeCredito, mAdeudo, Helper.DividirONull(mAdeudo, oTotalesCli.Adeudo)
                    , mVencido, Helper.DividirONull(mVencido, oTotalesCli.Vencido), oReg.PromedioDePagoAnual, oReg.PromedioDePago3Meses);
            }
            // Se orden el grid
        ...
        {
            var oPorSemana = this.AgruparPorEntero(oDatos.GroupBy(g => UtilLocal.SemanaSabAVie(g.Vencimiento.Valor())));
            ...
            foreach (var oReg in oPorSemana)
            {
                this.dgvPorSemana.Rows.Add(oReg.Llave, oReg.Esperado, oReg.Pagado
                    , Helper.DividirONull(oReg.Pagado, oReg.Esperado), Helper.DividirONull(oReg.Pagado, oReg.PagadoAnt));
