...
namespace Refaccionaria.App
{
    public partial class CuadroVip : UserControl
    {
        ...
        private void dgvClientes_CurrentCellChanged(object sender, EventArgs e)
        {
            ...
            {
                Cargando.Mostrar();
                int iClienteID = Helper.ConvertirEntero(this.dgvClientes.CurrentRow.Cells["cli_ClienteID"].Value);
        ...
        private void AplicarFormatoColumnas()
        {
            ...
            this.txtPromedioSemanas.Text = Helper.ConvertirDecimal(this.txtPromedioSemanas.Text).ToString(sFormato);
            this.txtPromedioMeses.Text = Helper.ConvertirDecimal(this.txtPromedioMeses.Text).ToString(sFormato);
            this.txtPromedioAnios.Text = Helper.ConvertirDecimal(this.txtPromedioAnios.Text).ToString(sFormato);
        }
        ...
        private void CargarClientes()
        {
            ...
            this.oDatos = General.ExecuteProcedure<pauCuadroDeControlGeneral_Result>("pauCuadroDeControlGeneral", oParams);
            // Se obtienen los datos para partes
            this.oDatosPartes = General.ExecuteProcedure<pauCuadroDeControlPartes_Result>("pauCuadroDeControlPartes", oParams);
            // Se obtienen los datos por año
            oParams.Remove("Desde");
            oParams.Remove("Hasta");
            this.oDatosPorAnio = General.ExecuteProcedure<pauCuadroDeControlPorAnio_Result>("pauCuadroDeControlPorAnio", oParams);
            ...
            {
                int iFila = this.dgvClientes.Rows.Add(oReg.Llave, oReg.Cadena, oReg.Actual, Helper.DividirONull(oReg.Actual, oReg.Anterior)
                    , Helper.DividirONull(oReg.Actual, mTotal));
            ...
            {
                mPorAcum += Helper.ConvertirDecimal(oFila.Cells["cli_Porcentaje"].Value);
        ...
        private void CargarSemanas(int iClienteID)
        {
            ...
            var oSemanas = this.AgruparPorEntero(this.oDatos.Where(c => c.ClienteID == iClienteID).GroupBy(c => UtilLocal.SemanaSabAVie(c.Fecha)));
            ...
            {
                int iFila = this.dgvSemanas.Rows.Add(oReg.Llave, oReg.Llave, oReg.Actual, Helper.DividirONull(oReg.Actual, oReg.Anterior)
                    , Helper.DividirONull(oReg.Actual, mTotal));
        ...
        private void CargarMeses(int iClienteID)
        {
            ...
            {
                int iFila = this.dgvMeses.Rows.Add(oReg.Llave, oReg.Llave, oReg.Actual, Helper.DividirONull(oReg.Actual, oReg.Anterior)
                    , Helper.DividirONull(oReg.Actual, mTotal));
        ...
        private void CargarSucursales(int iClienteID)
        {
            ...
            foreach (var oReg in oSucursales)
                this.dgvSucursales.Rows.Add(oReg.Llave, oReg.Cadena, oReg.Actual, Helper.DividirONull(oReg.Actual, oReg.Anterior), Helper.DividirONull(oReg.Actual, mTotal));
        }
        ...
        private void CargarVendedores(int iClienteID)
        {
            ...
            foreach (var oReg in oVendedores)
                this.dgvVendedores.Rows.Add(oReg.Llave, oReg.Cadena, oReg.Actual, Helper.DividirONull(oReg.Actual, oReg.Anterior), Helper.DividirONull(oReg.Actual, mTotal));
        }
        ...
        private void CargarArbolPartes(int iClienteID)
        {
            ...
            foreach (var oReg in oPartes)
            {
                ...
                {
                    sMarca = oReg.Marca;
                    decimal mPor = Helper.DividirONull(oReg.Actual, mTotal).Valor();
                    oNodoMarca = oNodoLinea.Nodes.Add(sMarca, oReg.Actual, mPor);
                    // Se llenan los totales de los niveles superiores
                    oNodoLinea.Cells[1].Value = (Helper.ConvertirDecimal(oNodoLinea.Cells[1].Value) + oReg.Actual);
                    oNodoLinea.Cells[2].Value = (Helper.ConvertirDecimal(oNodoLinea.Cells[2].Value) + mPor);
                    oNodoSubsistema.Cells[1].Value = (Helper.ConvertirDecimal(oNodoSubsistema.Cells[1].Value) + oReg.Actual);
                    oNodoSubsistema.Cells[2].Value = (Helper.ConvertirDecimal(oNodoSubsistema.Cells[2].Value) + mPor);
                    oNodoSistema.Cells[1].Value = (Helper.ConvertirDecimal(oNodoSistema.Cells[1].Value) + oReg.Actual);
                    oNodoSistema.Cells[2].Value = (Helper.ConvertirDecimal(oNodoSistema.Cells[2].Value) + mPor);
                }
            }
            ...
            {
                decimal mTotSis = Helper.ConvertirDecimal(oNodSistema.Cells[1].Value);
                foreach (var oNodSubsistema in oNodSistema.Nodes)
                {
                    decimal mTotSub = Helper.ConvertirDecimal(oNodSubsistema.Cells[1].Value);
                    oNodSubsistema.Cells[2].Value = Helper.DividirONull(Helper.ConvertirDecimal(oNodSubsistema.Cells[1].Value), mTotSis);
                    foreach (var oNodLinea in oNodSubsistema.Nodes)
                    {
                        decimal mTotLinea = Helper.ConvertirDecimal(oNodLinea.Cells[1].Value);
                        oNodLinea.Cells[2].Value = Helper.DividirONull(Helper.ConvertirDecimal(oNodLinea.Cells[1].Value), mTotSub);
                        foreach (var oNodMarca in oNodLinea.Nodes)
                        {
                            decimal mTotMarca = Helper.ConvertirDecimal(oNodMarca.Cells[1].Value);
                            oNodMarca.Cells[2].Value = Helper.DividirONull(Helper.ConvertirDecimal(oNodMarca.Cells[1].Value), mTotLinea);
                        }
                    }
                }
            }
        ...
        {
            var oGrupo = oDatos.GroupBy(c => new { c.ClienteID, Semana = UtilLocal.SemanaSabAVie(c.Fecha) });
