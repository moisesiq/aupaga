...
namespace Refaccionaria.App
{
    public partial class CuadroProveedores : CuadroMultiple
    {
        ...
        protected override void CargarDatos()
        {
            ...
            var oProveedores = this.AgruparPorEnteroCadena(oDatos.GroupBy(g => new EnteroCadena() { Entero = g.ProveedorID.Valor(), Cadena = g.Proveedor }))
        ...
        protected override void LlenarGrupos(int iId)
        {
            ...
            var oConsulta = this.AgruparPorEnteroCadena(oDatos.Where(c => c.ProveedorID == iId).GroupBy(g => new EnteroCadena() { Entero = g.LineaID.Valor(), Cadena = g.Linea }))
                .OrderByDescending(c => c.Actual);
        ...
        protected override void LlenarSemanas(int iId)
        {
            ...
            var oConsulta = this.AgruparPorEntero(oDatos.Where(c => c.ProveedorID == iId).GroupBy(g => UtilLocal.SemanaSabAVie(g.Fecha)));
            decimal mTotal = (oConsulta.Count() > 0 ? oConsulta.Sum(c => c.Actual) : 0);
            foreach (var oReg in oConsulta)
            {
        ...
        protected override void LLenarVendedores(int iId)
        {
            ...
            var oConsulta = this.AgruparPorEnteroCadena(oDatos.Where(c => c.ProveedorID == iId).GroupBy(g => new EnteroCadena() { Entero = g.VendedorID, Cadena = g.Vendedor }))
                .OrderByDescending(c => c.Actual);
            decimal mTotal = (oConsulta.Count() > 0 ? oConsulta.Sum(c => c.Actual) : 0);
            foreach (var oReg in oConsulta)
            {
        ...
        protected override void LlenarSucursales(int iId)
        {
            ...
            var oConsulta = this.AgruparPorEnteroCadena(oDatos.Where(c => c.ProveedorID == iId).GroupBy(g => new EnteroCadena() { Entero = g.SucursalID, Cadena = g.Sucursal }));
            decimal mTotal = (oConsulta.Count() > 0 ? oConsulta.Sum(c => c.Actual) : 0);
            foreach (var oReg in oConsulta)
            {
        ...
        protected override void LlenarPartes(int iGridFuente, int iIdPrincipal, int iId)
        {
            ...
            var oConsulta = oDatos.Where(c => c.ProveedorID == iIdPrincipal);
            switch (iGridFuente)
            {
        ...
        protected void CargarDatosCompras()
        {
            ...
            var oDatos = General.ExecuteProcedure<pauCuadroDeControlCompras_Result>("pauCuadroDeControlCompras", oParams);
        ...
        protected void LlenarSemanasCompras(int iId)
        {
            ...
            // Se llena el grid y la gráfica de Semanas
            var oConsulta = oDatos.Where(c => c.ProveedorID == iId).GroupBy(g => UtilLocal.SemanaSabAVie(g.Fecha.Valor())).Select(
        ...
        protected void LlenarPartesCompras(int iGridFuente, int iIdPrincipal, int iId)
        {
            ...
            switch (iGridFuente)
            {
                case CuadroMultiple.GridFuente.Grupos:
                    foreach (var oReg in oConsulta)
                    {
                        var oDetV = General.GetListOf<MovimientoInventarioDetalleView>(c => c.MovimientoInventarioID == oReg.MovimientoInventarioID && c.LineaID == iId);
            ...
            foreach (var oReg in oConsulta)
            {
                var oDetV = General.GetListOf<MovimientoInventarioDetalleView>(c => c.MovimientoInventarioID == oReg.MovimientoInventarioID);
                foreach (var oRegDet in oDetV)
                    this.dgvPartes.Rows.Add(oReg.Fecha, oReg.FolioFactura, oRegDet.NumeroParte, oRegDet.NombreParte
                        , (oRegDet.PrecioUnitarioConDescuento * oRegDet.Cantidad));
            }
