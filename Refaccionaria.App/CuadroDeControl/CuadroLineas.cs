...
namespace Refaccionaria.App
{
    public partial class CuadroLineas : CuadroMultiple
    {
        ...
        protected override void CargarDatos()
        {
            ...
            var oLineas = this.AgruparPorEnteroCadena(oDatos.GroupBy(g => new EnteroCadena() { Entero = g.LineaID.Valor(), Cadena = g.Linea }))
        ...
        protected override void LlenarGrupos(int iId)
        {
            ...
            // Se llena el grid de líneas
            var oConsulta = this.AgruparPorEnteroCadena(oDatos.Where(c => c.LineaID == iId).GroupBy(g => new EnteroCadena() { Entero = g.ClienteID, Cadena = g.Cliente }))
                .OrderByDescending(c => c.Actual);
        ...
        protected override void LlenarSemanas(int iId)
        {
            ...
            var oConsulta = this.AgruparPorEntero(oDatos.Where(c => c.LineaID == iId).GroupBy(g => UtilLocal.SemanaSabAVie(g.Fecha)));
            decimal mTotal = (oConsulta.Count() > 0 ? oConsulta.Sum(c => c.Actual) : 0);
            foreach (var oReg in oConsulta)
            {
        ...
        protected override void LLenarVendedores(int iId)
        {
            ...
            var oConsulta = this.AgruparPorEnteroCadena(oDatos.Where(c => c.LineaID == iId).GroupBy(g => new EnteroCadena() { Entero = g.VendedorID, Cadena = g.Vendedor }))
                .OrderByDescending(c => c.Actual);
            decimal mTotal = (oConsulta.Count() > 0 ? oConsulta.Sum(c => c.Actual) : 0);
            foreach (var oReg in oConsulta)
            {
        ...
        protected override void LlenarSucursales(int iId)
        {
            ...
            var oConsulta = this.AgruparPorEnteroCadena(oDatos.Where(c => c.LineaID == iId).GroupBy(g => new EnteroCadena() { Entero = g.SucursalID, Cadena = g.Sucursal }));
            decimal mTotal = (oConsulta.Count() > 0 ? oConsulta.Sum(c => c.Actual) : 0);
            foreach (var oReg in oConsulta)
            {
