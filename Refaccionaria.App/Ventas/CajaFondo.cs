...
namespace Refaccionaria.App
{
    public partial class CajaFondo : CajaMonedas
    {
        ...
        private void CajaFondo_Load(object sender, System.EventArgs e)
        {
            ...
            var oEfectivos = General.GetListOf<CajaEfectivoPorDia>(q => q.SucursalID == GlobalClass.SucursalID && q.Dia < dHoy && q.Estatus);
            if (oEfectivos.Count > 0)
            {
                int iEfectivoID = oEfectivos.Max(q => q.CajaEfectivoPorDiaID);
                oEfectivo = General.GetEntity<CajaEfectivoPorDia>(q => q.CajaEfectivoPorDiaID == iEfectivoID);
        ...
        public void ActualizarDatos()
        {
            ...
            DateTime dHoy = DateTime.Today;
            var oEfectivo = General.GetEntity<CajaEfectivoPorDia>(q => q.SucursalID == GlobalClass.SucursalID && q.Dia == dHoy && q.Estatus);
            if (oEfectivo != null)
            {
