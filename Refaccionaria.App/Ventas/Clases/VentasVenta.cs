...
namespace Refaccionaria.App
{
    class VentasVenta
    {
        ...
        public bool Ejecutar()
        {
            ...
            {
                if (General.Exists<Parte>(c => c.ParteID == oReg.ParteID && c.EsCascoPara > 0) || oReg.ParteID == Cat.Partes.DiferenciaDeCascos)
                {
            ...
            if (!this.oControlVentas.ctlCobro.CompletarCobro())
                return false;
            decimal mImportePato = Helper.ConvertirDecimal(Config.Valor("Ventas.ImportePato"));
            bool esPato = false;
            ...
            */
            ...
            foreach (var oReg in oVentaDetalle)
            {
                if (General.Exists<Parte>(c => c.ParteID == oReg.ParteID && c.RequiereCascoDe > 0 && c.Estatus))
                {
                    ...
                    for (int i = 0; i < oReg.Cantidad; i++)
                    {
                        ...
                        Guardar.Generico<CascoRegistro>(oCascoReg);
                    }
                }
            }
            ...
            if (this.oControlVentas.oAplicaciones != null && this.oControlVentas.oAplicaciones.Count > 0)
            {
                foreach (var oReg in this.oControlVentas.oAplicaciones)
                {
                    ...
                    Guardar.Generico<VentaParteAplicacion>(oReg);
                }
            }
        ...
        public void GenerarCotizacion()
        {
            ...
            {
                oParte = General.GetEntity<Parte>(q => q.ParteID == oDet.ParteID && q.Estatus);
