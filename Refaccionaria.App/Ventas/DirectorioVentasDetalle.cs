    ...
    public partial class DirectorioVentasDetalle : UserControl
    {
        ...
        internal void AddFiles(int LineaMarcaId)
        {
            ...
            if (cliente == eModo.Lineas)
            {
                ...
                var Archivo = General.GetListOf<LineaAttachFile>(p => p.LineaID == LineaMarcaId).ToList();
            ...
            else
            {
                ...
                var Archivo = General.GetListOf<MarcaAttachFile>(p => p.MarcaID == LineaMarcaId).ToList();
                foreach (var oAttach in Archivo)
                {
                    string tipo = this.tipoArchivo(oAttach.NombreArchivo);
        ...
        {
            return Helper.ConvertirEntero(Elemento.SubItems[0].Text);
        }
        ...
        public void llenaCampos(int IdProveedor, string NombreProveedor)
        {
            ...
            else
            {
                ...
                Refaccionaria.Negocio.Controles.ControlPicture.CargaImagen(strLogo, this.picLogo);
                ...
                var oProvee = General.GetListOf<ProveedorContactosView>(p => p.ProveedorID == IdProveedor);
                ...
                var Obs = General.GetEntity<ProveedorObservacion>(p => p.ProveedorID == IdProveedor);
