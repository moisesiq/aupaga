...
namespace Refaccionaria.App
{
    public partial class Detalle9500Com : cVentaDetalleMod
    {
            ...
            switch (e.KeyCode)
            {
                ...
                case Keys.Enter:
                    ...
                    if (frmCantidad.ShowDialog(Principal.Instance) == DialogResult.OK)
                        this.ModificarCantidad(this.dgvProductos.SelectedRows[0], Helper.ConvertirEntero(frmCantidad.Valor));
                    frmCantidad.Dispose();
                    e.Handled = true;
                    break;
            }
        ...
        private bool ValidarExistencia(DataGridViewRow Fila, decimal mCantidad)
        {
            ...
            var ParteV = General.GetEntity<PartesVentasView>(q => q.ParteID == oProductoV.ParteID);
            if (ParteV == null)
                return false;
        ...
        public bool VerExistenciaLista()
        {
            ...
            foreach (DataGridViewRow Fila in this.dgvProductos.Rows)
            {
                if (!Helper.ConvertirBool(Fila.Cells["Aplicar"].Value)) continue;
