...
namespace Refaccionaria.App
{
    public partial class CajaConteoPagos : CajaMonedas
    {
        ...
        {
            bool bMarcado = Helper.ConvertirBool(Fila.Cells["Resguardar"].Value);
            decimal mImporte = Helper.ConvertirDecimal(Fila.Cells["Importe"].Value);
            switch (Helper.ConvertirEntero(Fila.Cells["FormaDePagoID"].Value))
            {
        ...
        public void ActualizarDatos()
        {
            ...
            var oPagos = General.GetListOf<VentasPagosDetalleView>(q => q.SucursalID == iSucursalID && EntityFunctions.TruncateTime(q.Fecha) == dHoy);
            ...
            decimal mImporteIdeal = Helper.ConvertirDecimal(Config.Valor("Caja.Resguardo.Ideal"));
        ...
        public virtual ConteoCaja GenerarConteo()
        {
            ...
            foreach (DataGridViewRow Fila in this.dgvPagosBancarios.Rows)
            {
                if (Helper.ConvertirBool(Fila.Cells["Resguardar"].Value))
                    ...
                    {
                        Tipo = Helper.ConvertirCadena(Fila.Cells["Tipo"].Value),
                        Importe = Helper.ConvertirDecimal(Fila.Cells["Importe"].Value),
                        Banco = Helper.ConvertirCadena(Fila.Cells["Banco"].Value),
                        Cuenta = Helper.ConvertirCadena(Fila.Cells["Cuenta"].Value),
                        Folio = Helper.ConvertirCadena(Fila.Cells["Folio"].Value)
                    });
            }
