...
namespace Refaccionaria.App
{
    public partial class Inventario : UserControl
    {
        ...
        private void Inventario_Load(object sender, EventArgs e)
        {
            ...
            // Resultados
            this.cmbResultadosLineas.CargarDatos("LineaID", "Linea", General.GetListOf<InventarioLineasAgrupadoView>(c => c.EstatusGenericoID == Cat.EstatusGenericos.Completada));
            this.cmbResultadosCasos.Items.AddRange(new object[] { "TRASPASOS", "FALTANTES Y SOB.", "SIN DIFERENCIA" });
            // Asignación manual
            this.cmbLinea.CargarDatos("LineaID", "NombreLinea", General.GetListOf<Linea>(c => c.Estatus).OrderBy(o => o.NombreLinea).ToList());
        ...
        private void LlenarProgreso()
        {
            ...
            var oDatos = General.ExecuteProcedure<pauInventarioProgreso_Result>("pauInventarioProgreso", oParams);
            var oPeriodicidad = General.GetListOf<InventarioLineaPeriodicidad>();
            ...
            foreach (var oReg in oDatos)
            {
                ...
                if (Helper.ConvertirFechaHora(this.dgvProgreso["Progreso_FechaCompletado", iFila].Value) > dAvance)
                    dAvance = Helper.ConvertirFechaHora(this.dgvProgreso["Progreso_FechaCompletado", iFila].Value);
            ...
            oParams["Opcion"] = 2;
            oDatos = General.ExecuteProcedure<pauInventarioProgreso_Result>("pauInventarioProgreso", oParams);
            ...
            while (dAvance <= this.dtpHasta.Value)
            {
                ...
                /* if (iExistentes >= iReg)
                {
                    ...
                    if (oReg.LineaID == Helper.ConvertirEntero(this.dgvProgreso["Progreso_LineaID", iReg - 1].Value)
                        && (Helper.ConvertirFechaHora(this.dgvProgreso["Progreso_FechaIniciado", iReg - 1].Value) - dAvance).Days < 2)
                        continue;
                    dAvance = Helper.ConvertirFechaHora(this.dgvProgreso["Progreso_FechaCompletado", iFila].Value);
                }
                */
        ...
        private void LlenarResultados()
        {
            ...
            int iLineaID = Helper.ConvertirEntero(this.cmbResultadosLineas.SelectedValue);
            int iOpcion = this.cmbResultadosCasos.SelectedIndex;

            var oDatos = General.GetListOf<InventarioResultadosView>(c => c.LineaID == iLineaID
            ...
            switch (sColumna)
            {
                case "Resultados_DecisionMatriz":
                    oFila.Cells["Resultados_ImporteMatriz"].Value =
                        ((Helper.ConvertirDecimal(oFila.Cells["Resultados_DiferenciaMatriz"].Value) + Helper.ConvertirDecimal(oFila.Cells["Resultados_DecisionMatriz"].Value))
                        * Helper.ConvertirDecimal(oFila.Cells["Resultados_Costo"].Value));
                    break;
                case "Resultados_DecisionSuc2":
                    oFila.Cells["Resultados_ImporteSuc2"].Value =
                        ((Helper.ConvertirDecimal(oFila.Cells["Resultados_DiferenciaSuc2"].Value) + Helper.ConvertirDecimal(oFila.Cells["Resultados_DecisionSuc2"].Value))
                        * Helper.ConvertirDecimal(oFila.Cells["Resultados_Costo"].Value));
                    break;
                case "Resultados_DecisionSuc3":
                    oFila.Cells["Resultados_ImporteSuc3"].Value =
                        ((Helper.ConvertirDecimal(oFila.Cells["Resultados_DiferenciaSuc3"].Value) + Helper.ConvertirDecimal(oFila.Cells["Resultados_DecisionSuc3"].Value))
                        * Helper.ConvertirDecimal(oFila.Cells["Resultados_Costo"].Value));
                    break;
            }
        }
        ...
        private void CargarAsignacion()
        {
            ...
            var oCantidad = General.GetListOf<InventarioLineasPartesView>(c => c.LineaID == iLineaID);
            var oCapacidad = General.GetListOf<InventarioUsuariosView>().OrderBy(o => o.SucursalID).ToList();
        ...
        private void GuardarAsignacion()
        {
            ...
            {
                string sSucursal = Helper.ConvertirCadena(oFila.Cells["Asignacion_Sucursal"].Value);
                oSucPartes[sSucursal] = Helper.ConvertirEntero(oFila.Cells["Asignacion_Partes"].Value);
                if (!oSucCant.ContainsKey(sSucursal))
                    oSucCant.Add(sSucursal, 0);
                oSucCant[sSucursal] += Helper.ConvertirEntero(oFila.Cells["Asignacion_Cantidad"].Value);
            }
            ...
            int iLineaID = Helper.ConvertirEntero(this.cmbLinea.SelectedValue);
            var oAvManual = General.GetListOf<InventarioLinea>(c => c.AvManual > 0);
            int iAvManual = (oAvManual.Count > 0 ? (oAvManual.Max(c => c.AvManual).Valor() + 1) : 1);
            var oPartes = General.GetListOf<Parte>(c => c.LineaID == iLineaID && c.Estatus);
            int iParte = 0, iSucursalID = 0, iCantidad, iUsuarioID;
            var oInvLineas = new Dictionary<int, int>();
            DateTime dAhora = DateTime.Now;
            foreach (DataGridViewRow oFila in this.dgvAsignacionManual.Rows)
            {
                if (Helper.ConvertirEntero(oFila.Cells["Asignacion_SucursalID"].Value) != iSucursalID)
                {
                    iSucursalID = Helper.ConvertirEntero(oFila.Cells["Asignacion_SucursalID"].Value);
                    iParte = 0;
                }
                iCantidad = Helper.ConvertirEntero(oFila.Cells["Asignacion_Cantidad"].Value);
                iUsuarioID = Helper.ConvertirEntero(oFila.Cells["Asignacion_UsuarioID"].Value);
                while (iCantidad > 0 && iParte < oPartes.Count)
                {
                    ...
                    if (!oInvLineas.ContainsKey(iSucursalID))
                    {
                        ...
                        Guardar.Generico<InventarioLinea>(oInvLinea);
                    ...
                    Guardar.Generico<InventarioConteo>(oConteo);
