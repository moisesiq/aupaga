...
namespace Refaccionaria.App
{
    public partial class DetalleProveedorPoliza : DetalleBase
    {
        ...
        private void AnalizarNotasDeCredito()
        {
            ...
            foreach (DataGridViewRow oFila in this.dgvDetalle.Rows)
                oMovsAPagar.Add(Helper.ConvertirEntero(oFila.Cells["fac_MovimientoInventarioID"].Value));

            var oNotas = General.GetListOf<ProveedorNotaDeCredito>(c => c.ProveedorID == this.Proveedor.ProveedorID && c.Disponible);
            var oMovsNotas = new List<int>();
            foreach (var oReg in oNotas)
            {
                oMovsNotas.AddRange(General.GetListOf<ProveedorNotaDeCreditoDetalle>(c => c.ProveedorNotaDeCreditoID == oReg.ProveedorNotaDeCreditoID)
            ...
            foreach (var iMov in oMovsNotas)
            {
                ...
                if (!bNotasPag)
                {
                    if (General.Exists<MovimientoInventario>(c => c.MovimientoInventarioID == iMov && c.FueLiquidado))
                    {
                        bNotasPag = true;
                    }
                }
