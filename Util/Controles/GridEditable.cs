...
namespace LibUtil
{
    public class GridEditable : DataGridView
    {
        ...
        protected virtual void Evento_RowsAdded(object sender, DataGridViewRowsAddedEventArgs e)
        {
            ...
            if (Util.Entero(oFila.Cells["__Cambio"].Value) == Con.Afectaciones.NoEspecificado)
            {
                oFila.Cells["__Cambio"].Value = Con.Afectaciones.Agregar;
        ...
        protected virtual void Evento_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            {
                if (Util.Entero(this.CurrentRow.Cells["__Cambio"].Value) == Con.Afectaciones.Agregar)
                {
                    this.Rows.Remove(this.CurrentRow);
                }
                else if (Util.Entero(this.CurrentRow.Cells["__Cambio"].Value) == Con.Afectaciones.Borrar)
                {
                    this.CurrentRow.Cells["__Cambio"].Value = Con.Afectaciones.Modificar;
                    this.CurrentRow.DefaultCellStyle.ForeColor = this.ColorModificado;
                }
                else
                {
                    this.CurrentRow.Cells["__Cambio"].Value = Con.Afectaciones.Borrar;
        ...
        protected virtual void Evento_CellValueChanged(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (Util.Entero(oFila.Cells["__Cambio"].Value) == Con.Afectaciones.SinCambios)
            {
                oFila.Cells["__Cambio"].Value = Con.Afectaciones.Modificar;
        ...
        public void Inicializar()
        {
            ...
            if (this.Columns.Count == 0)
                Util.MensajeAdvertencia("El Grid Editable no tiene ninguna columna al momento de inicializar. Verificar.", "Advertencia");
        ...
        {
            return Util.Entero(oFila.Cells["__Id"].Value);
        }
        ...
        {
            return Util.Entero(oFila.Cells["__Cambio"].Value);
        }
